...
def initialize_linkedin_video_upload(access_token, video_initialize_upload_url, video_url):
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info("Linkedin video upload successfully initiated...")
            return response_json
        else:
            error_message = f"Linkedin video upload could not be initiated: {response_json.get('message', 'Unknown error')}"
            logger.info(error_message)
            error = {"error": error_message}
            return error

    except Exception as e:
        error_message = f"Linkedin video upload could not be initiated: {str(e)}"
        logger.info(error_message)
        error = {"error": error_message}
        return error
...
def upload_video_to_linkedin(access_token, video_url, upload_urls):
    ...
    try:
        ...
        for chunk in chunks:
            ...
            logger.info(f"Chunk {chunk_number}, Status Code: {response.status_code}, etag: {response.headers.get('etag')}")
            ...
            if chunk_number > len(upload_urls):
                break
        logger.info("Uploading video to linkedin server successful...")

        return {'uploaded_part_ids': uploaded_part_ids}

    except Exception as e:
        error_message = f"Uploading video to linkedin server failed: {str(e)}"
        logger.info(error_message)
        error = {"error": error_message}
        return error
...
def finalize_linkedin_video(access_token, video_finalize_upload_url, final_video_urn, uploaded_part_ids):
    ...
    if response.status_code == 200:
        logger.info("Linkedin video upload completed...")
        return {"video_upload_finalize": "success"}

    else:
        logger.info("Linkedin video upload failed...")
        return {
...
def create_linkedin_post(access_token, author, final_video_urn, post, url):
    ...
    logger.info("Creating linkedin video post...")
    ...
    try:
        ...
        if response.status_code != 201:
            logger.info("Linkedin video post could not be created...")
            error = {
            ...
            logger.info("Linkedin video post successfully created...")
