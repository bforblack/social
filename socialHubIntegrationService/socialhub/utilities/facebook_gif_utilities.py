...
def create_gif_upload_session(url, access_token, file_size):
    logger.info("Creating facebook gif upload session...")
    ...
    if not response.json().get('error'):
        logger.info("Facebook gif upload session successfully created...")
        return response.json()

    else:
        error_message = f"Facebook gif upload session could not be created: {response.json().get('error')}"
...
def upload_gif(url, upload_session_id, access_token, start_offset, gif_file):
    ...
    logger.info("Uploading gif to facebook server...")
    ...
    if not response.json().get('error'):
        logger.info("gif upload to facebook server completed successfully..")
        return response.json()
...
def finish_gif_upload_session(url, access_token, upload_session_id, post, gif_title):
    ...
    logger.info("Finishing facebook gif upload session and creating gif post...")
    ...
    if not response.json().get('error'):
        logger.info("Facebook gif upload session ended and gif post created successfully...")
        return response.json()
